...
export async function getAllPrompts(category?: string) {
  if (category) {
    return db.select().from(prompts).where(eq(prompts.category, category)).orderBy(desc(prompts.upvotes));
  }
...
export async function upvotePrompt(promptId: number, userId: string) {
  ...
  const existingUpvote = await db
    ...
    .where(
      and(
        eq(userUpvotes.promptId, promptId),
        eq(userUpvotes.userId, userId)
      )
    );
